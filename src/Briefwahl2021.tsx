...
const Briefwahl2021 = () => {
  ...
  const StateDataArray: any = [
    { Title: 'Alle', src: 'https://gruene-weltweit.de/assets/Alle.png', IsSelected: false },
    ...
    try {
      ...
      fetch("https://gruene-weltweit.de/SPPublicAPIs/getDataAll.php", requestOptions)
        ...
        .then((result: any) => {
          ...
          if (State != undefined && State != undefined && State.toLowerCase() == 'Alle') {
  ...
  const ChangeTile = (tile: string) => {
    ...
    if (tile != 'Alle' && tile.toLowerCase() != 'Alle') {
      BriefwahldataBackup = allfilterdata;
    } else if (tile != undefined && tile != undefined && tile.toLowerCase() == 'Alle') {
